...
import { INFLUX_ADDRESS, INFLUX_BUCKET, INFLUX_ORG, INFLUX_TOKEN } from "./secrets.js";

async function get_table_time_range (table, start_time, stop_time, result) {
    const client = new InfluxDB({url: INFLUX_ADDRESS, token: INFLUX_TOKEN});
    ...
    await get_objs().catch((error) => {result(error, null); return})
...
export { get_last_n_from_table, get_table_time_range };
